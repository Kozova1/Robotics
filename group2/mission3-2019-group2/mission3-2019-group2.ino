...
#define NEOPIXEL_DATAPIN A0
#define NEOPIXELS 16

MPU6050 gyro_mpu;

Adafruit_NeoPixel pixels = Adafruit_NeoPixel(NEOPIXELS, NEOPIXEL_DATAPIN, NEO_RGB + NEO_KHZ800);
...
void setup() 
{
  ...
  while(!gyro_mpu.begin(MPU6050_SCALE_2000DPS, MPU6050_RANGE_2G))
  {
...
void checkSettings()
{
  ...
  Serial.println(gyro_mpu.getSleepEnabled() ? "Enabled" : "Disabled");
  
  Serial.print(" * Clock Source:          ");
  switch(gyro_mpu.getClockSource()){
  ...
  Serial.print(gyro_mpu.getAccelOffsetX());
  Serial.print(" Y: ");
  Serial.print(gyro_mpu.getAccelOffsetY());
  Serial.print(" Z: ");
  Serial.println(gyro_mpu.getAccelOffsetZ());
...
void loop()
{
  Vector normAccel = gyro_mpu.readNormalizeAccel();
